...
def calc_all(inp):
    ...
    pie = [
        {"name": "Kaufpreis", "value": inp['kaufpreis'], "color": T["primary"]},
        {"name": "Grunderwerbsteuer", "value": grunderwerbsteuer, "color": "#0ea5e9"},
        {"name": "Notar/Grundbuch", "value": notar, "color": "#8b5cf6"},
        {"name": "Makler", "value": makler, "color": "#14b8a6"}
    ]
...
AMP = {
    "green": {"color": T["green"], "bg": "rgba(5,150,105,0.1)", "label": "Gut", "border": "rgba(5,150,105,0.2)"},
    "yellow": {"color": T["yellow"], "bg": "rgba(217,119,6,0.1)", "label": "Akzeptabel", "border": "rgba(217,119,6,0.2)"},
    "red": {"color": T["red"], "bg": "rgba(220,38,38,0.1)", "label": "Kritisch", "border": "rgba(220,38,38,0.2)"}
}
...
def render_kpi(label, value, sub_label="", ampel_key=None, ampel_val=None):
    ...
    if ampel_key:
        ...
        val_color = a["color"]
        border_top = f"border-top: 3px solid {a['color']};"
        ampel_html = f"<div class='ampel-badge' style='color:{a['color']}; background:{a['bg']}; border: 1px solid {a['border']};'>{a['label']}</div>"
...
with col_logo:
    st.markdown(f"<h2 style='margin:0; font-weight:700; color:{T['text']};'><span style='color:{T['primary']};'>YieldBase</span> Analytics</h2>", unsafe_allow_html=True)
with col_mode:
...
with t1:
    ...
    with col_r:
        # Kernergebnis-Banner (Light SaaS Style)
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 1px solid #bfdbfe; border-radius: 16px; padding: 28px 32px; margin-bottom: 24px; box-shadow: 0 4px 6px -1px rgba(37,99,235, 0.05);">
            <div style="font-weight: 500; font-size: 0.9rem; color: #1e40af; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em;">Gesamtinvestition (inkl. {fmt_pct(inp['knk'])} KNK)</div>
            <div style="font-family: 'JetBrains Mono', monospace; font-size: 2.6rem; font-weight: 700; color: #1e3a8a; letter-spacing: -0.02em;">{fmt_eur(res['gesamt'])}</div>
            <div style="display: flex; gap: 30px; margin-top: 18px;">
                <div><div style="font-size: 0.75rem; color: #3b82f6; text-transform: uppercase; font-weight: 600;">Eigenkapital</div><div style="font-family: 'JetBrains Mono', monospace; color: #1e40af; font-weight: 700; font-size: 1.1rem;">{fmt_eur(res['ekAbs'])}</div></div>
                <div><div style="font-size: 0.75rem; color: #3b82f6; text-transform: uppercase; font-weight: 600;">Fremdkapital</div><div style="font-family: 'JetBrains Mono', monospace; color: #1e40af; font-weight: 700; font-size: 1.1rem;">{fmt_eur(res['fk'])}</div></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not is_expert:
            ...
            st.markdown(f"<div class='benchmark-box'>💡 <strong>Kaufpreisfaktor: {fmt_x(res['faktor'])}</strong> — {msg}</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"<div class='saas-card' style='margin-top:24px;'><div class='saas-card-header'>Kostenstruktur</div>", unsafe_allow_html=True)
        ...
        pie = base.mark_arc(innerRadius=60, outerRadius=90)
        ...
        with col_p2:
            st.markdown("<br>", unsafe_allow_html=True)
            for item in res['pie']:
                st.markdown(f"<div style='display:flex; justify-content:space-between; margin-bottom:10px; border-bottom:1px solid {T['surface3']}; padding-bottom:4px;'><span style='color:{T['textMuted']}; font-size:0.9rem; font-weight:500;'><span style='display:inline-block; width:12px; height:12px; background:{item['color']}; border-radius:3px; margin-right:10px; transform:translateY(1px);'></span>{item['name']}</span><span style='font-family:\"JetBrains Mono\", monospace; font-weight:600; color:{T['text']};'>{fmt_eur(item['value'])}</span></div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
with t3:
    ...
    st.area_chart(df_proj[['CF pre-Tax', 'CF post-Tax']], color=[T["cyan"], T["primary"]], height=300)
    ...
    <div style="background: {T['surface']}; border: 1px solid {T['border']}; border-radius: 16px; padding: 36px; margin-bottom: 20px; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.05);">
        ...
        <div style="display: flex; gap: 24px; margin-bottom: 30px;">
            <div style="background: {T['surface2']}; border-radius: 12px; padding: 20px; flex: 1; border: 1px solid {T['border']};">
                <div style="font-size: 0.75rem; color: {T['textDim']}; text-transform: uppercase; font-weight: 600; letter-spacing: 0.05em; margin-bottom: 4px;">Gesamtinvestition</div>
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 1.8rem; font-weight: 700; color: {T['primary']};">{fmt_eur(res['gesamt'])}</div>
            </div>
            <div style="background: {T['surface2']}; border-radius: 12px; padding: 20px; flex: 1; border: 1px solid {T['border']};">
                <div style="font-size: 0.75rem; color: {T['textDim']}; text-transform: uppercase; font-weight: 600; letter-spacing: 0.05em; margin-bottom: 4px;">Jahresmiete (Brutto)</div>
                <div style="font-family: 'JetBrains Mono', monospace; font-size: 1.8rem; font-weight: 700; color: {T['primary']};">{fmt_eur(inp['jahresmiete'])}</div>
            </div>
        </div>
        
        <div style="background: {T['primaryFaint']}; border-radius: 12px; padding: 20px; font-size: 1.05rem; line-height: 1.6; color: {T['textMuted']}; border-left: 4px solid {T['primary']};">
            <strong style="color: {T['text']}; font-weight: 600;">Analyse:</strong> Bei einem Kaufpreis von <strong style="color: {T['primary']}">{fmt_eur(inp['kaufpreis'])}</strong> und einer 
            Jahresnettomiete von <strong style="color: {T['primary']}">{fmt_eur(inp['jahresmiete'])}</strong> ergibt sich ein 
